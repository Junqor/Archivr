...
export class Game extends Scene {
  ...
  update(_time: number, delta: number): void {
    ...
    if (
      ...
      this.game_state == GAME_STATE.BALL_SHOOT
    ) {
  ...
  create() {
    ...
    this.physics.world.addCollider(this.Bricks, this.Balls, (brick, _ball) => {
      ...
      if (Brick.anims.getName() == "brick_0") {
        ...
        this.createPickup(
          Brick.body.position.x + 12.5,
          Brick.body.position.y + 7.5
        );
        Brick.destroy();
        if (
          this.game_state == GAME_STATE.PLAYING &&
          this.Bricks?.getLength() == 0
        ) {
    ...
    this.physics.world.addCollider(this.Paddle, this.Balls, (paddle, ball) => {
      ...
      const d =
        (b.body.position.x - 37.5 - p.body.position.x) * this.ball_angle_coeff;

      b.setVelocityX(
        d * this.paddle_angle_influence +
          b.body.velocity.x * (1 - this.paddle_angle_influence)
      );
    ...
    this.physics.world.addCollider(this.Pickups, this.Paddle, (_, pickup) => {
      ...
      if (score) score.innerHTML = `${parseInt(score.innerHTML) + 1}`;
  ...
  onWorldBounds(
    ...
    if (Scene.Balls.contains(body.gameObject)) {
      if (down) {
        ...
        if (
          Scene.game_state == GAME_STATE.PLAYING &&
          Scene.Balls.getLength() == 0
        ) {
  ...
  changeGameState(new_state: GAME_STATE) {
    ...
    switch (new_state) {
        ...
        this.SceneText.add(
          this.add
            .text(
              200,
              125,
              "LEVEL " + (this.current_level_index + 1).toString(),
              {
                fontFamily: "Arial Black",
                fontSize: 28,
                color: "#ffffff",
        ...
        this.SceneText.add(
          this.add
            .text(
              200,
              175,
              this.current_level ? this.current_level.title : "",
              {
                fontFamily: "Arial Black",
                fontSize: 20,
                color: "#ffffff",
      ...
      case GAME_STATE.LEVEL_END:
        ...
        this.Pickups.children.iterate(
          (pickup: Phaser.GameObjects.GameObject) => {
            ...
            if (Pickup) {
              ...
              Pickup.setVelocity(
                Scene.Paddle.body.position.x + 37.5 - Pickup.body.position.x,
                Scene.Paddle.body.position.y - Pickup.body.position.y
              );
            }
            return null;
          }
        ...
        const purity =
          Math.ceil(
            (this.level_stats_collected / this.level_stats_total_bricks) * 1000
          ) / 10;
        this.SceneText.add(
          this.add
            .text(
              200,
              175,
              `COOK: ${purity}% Pure (${this.level_stats_collected}/${this.level_stats_total_bricks})`,
              {
                fontFamily: "Arial Black",
                fontSize: 20,
                color: "#ffffff",
  ...
  createBrick(x: number, y: number) {
    if (!this.Bricks) return;
    let Brick = this.physics.add.staticSprite(
      12.5 + x * 25,
      7.5 + y * 15,
      "brick"
    );
