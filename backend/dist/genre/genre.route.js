...
import { get_popular_media_genre, get_media_genre, get_genres, } from "./genre.service.js";
...
genreRouter.get("/popular", async (req, res) => {
    try {
        ...
        res.status(400).json({
            status: "failed",
            message: err instanceof ZodError ? "Invalid query" : err.message,
        });
    }
});
...
genreRouter.get("/media", async (req, res) => {
    try {
        const { genre, offset, sortBy, order } = getMediaGenreBodySchema.parse(req.query);
